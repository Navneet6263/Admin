...
async function refresh() { const [stock, history] = await Promise.all([request<Record<string, unknown>[]>('/api/inventory'), request<Record<string, unknown>[]>('/api/inventory/movements')]); items = stock.map(asItem); movements = history.map(asMovement); notify(); }
...
export function useInventory() { const [snapshot, setSnapshot] = useState(items); useEffect(() => { void inventoryStore.refresh(); return inventoryStore.subscribe(() => setSnapshot([...items])); }, []); return snapshot; }
export function useStockMovements() { const [snapshot, setSnapshot] = useState(movements); useEffect(() => { void inventoryStore.refresh(); return inventoryStore.subscribe(() => setSnapshot([...movements])); }, []); return snapshot; }
