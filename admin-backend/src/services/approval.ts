...
export async function completeApproval(requestId: number, actorId: number, note = "") {
  ...
  try {
    const result = await tx.request().input("id", mssql.Int, requestId)
      .query(`SELECT user_id,ref_id,type,amount,actual_amount,payment_status,details,inventory_center_code,
    ...
    let paymentStatus = needsPayment ? (hasAmount ? "awaiting_verification" : "awaiting_update") : "not_required";
    let paymentAmount = hasAmount ? Number(request.amount) : null;
    let shouldBookExpense = needsPayment && paymentAmount != null;
    let existingPayment: Record<string, unknown> | undefined;
    if (needsPayment) {
      const existing = await tx.request().input("rid", mssql.Int, requestId)
        .query(`SELECT TOP 1 status,actual_amount,expense_booked FROM payments WITH(UPDLOCK,ROWLOCK)
          WHERE request_id=@rid`);
      existingPayment = existing.recordset[0];
      const existingActual = existingPayment?.actual_amount;
      if (existingActual !== null && existingActual !== undefined) {
        paymentAmount = Number(existingActual);
        paymentStatus = String(existingPayment?.status || "") === "paid" ? "paid" : "awaiting_verification";
      }
      if (String(existingPayment?.status || "") === "paid") paymentStatus = "paid";
      shouldBookExpense = paymentAmount != null && !Boolean(existingPayment?.expense_booked);
    }
    ...
    await tx.request()
      ...
      .input("actual", mssql.Decimal(14, 2), paymentAmount)
    ...
    if (needsPayment) {
      await tx.request()
        ...
        .input("estimated", mssql.Decimal(14, 2), hasAmount ? request.amount : null)
        .input("amount", mssql.Decimal(14, 2), paymentAmount)
        .input("status", mssql.NVarChar(30), paymentStatus)
        .input("due", mssql.DateTime2, due)
        .input("actor", mssql.Int, actorId)
        .input("booked", mssql.Bit, shouldBookExpense ? 1 : 0)
        .query(`IF EXISTS(SELECT 1 FROM payments WHERE request_id=@rid)
          UPDATE payments SET estimated_amount=COALESCE(estimated_amount,@estimated),
            actual_amount=COALESCE(actual_amount,@amount),status=CASE WHEN status='paid' THEN status ELSE @status END,
            due_at=COALESCE(due_at,@due),updated_by=COALESCE(updated_by,@actor),
            expense_booked=CASE WHEN @booked=1 THEN 1 ELSE expense_booked END,updated_at=SYSUTCDATETIME()
            WHERE request_id=@rid;
          ELSE INSERT INTO payments(request_id,estimated_amount,actual_amount,status,due_at,updated_by,expense_booked)
            VALUES(@rid,@estimated,@amount,@status,@due,@actor,@booked)`);
      if (shouldBookExpense) await tx.request()
  ...
  if (request.payment_status !== "not_required" && request.payment_status !== "paid") notices.push(
