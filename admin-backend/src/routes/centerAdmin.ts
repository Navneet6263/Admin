...
const router = Router();
router.use(requireAuth('admin', 'super_admin'));

// ── GET /api/center-admin/requests?status=pending ────────────
router.get('/requests', async (req: Request, res: Response) => {
  const centerCode = (req.user as any)?.center_code || 'A11';
    ...
    const result = await pool.request()
      ...
      .query(`
        SELECT TOP 200
          ...
          ISNULL(r.home_center_code, 'A11') AS home_center_code,
          r.fulfil_center_code,
          u.name AS employeeName, u.email, ISNULL(u.dept, r.team) AS employeeDept
        FROM requests r
        LEFT JOIN users u ON r.user_id = u.id
        WHERE (@status = 'all' OR r.status = @status)
          AND (r.home_center_code = @cc OR @cc = 'A11' OR @cc = 'ALL' OR r.home_center_code IS NULL)
        ORDER BY
...
router.get('/budget', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await pool.request()
      ...
      .query(`
        SELECT ISNULL(cb.allocated, 500000) AS allocated,
               ISNULL(cb.committed, 45000)  AS committed,
               ISNULL(cb.spent, 120000)     AS spent,
               ISNULL(c.name, 'Noida HQ (A11)') AS center_name,
               ISNULL(c.city, 'Noida')         AS city
        FROM centers c
    ...
    const result = await pool.request()
      ...
      .query(`
        SELECT
          ...
          COUNT(*) AS total,
          ISNULL(AVG(DATEDIFF(HOUR, created_at, ISNULL(updated_at, GETDATE()))), 2) AS avg_response_hrs
        FROM requests
        WHERE (home_center_code = @cc OR @cc = 'A11' OR @cc = 'ALL' OR home_center_code IS NULL)
      `);
